...
(function (app) {
    ...
    app.controller("DragAndDropController", ['$rootScope' , '$scope', 'dragAndDrop', '$document' , function ($rootScope, $scope, dragAndDrop, $document) {
        ...
        $scope.$on($DESTROY, function () {
            console.log("d&d $scope destroyed");
        ...
        return {
            ...
            link: function (scope, element, attrs) {
                ...
                var mouseDown = function (evt) {
                    window.ON_ELEMENT_CLICK_FOR_DRAG = true;
                    angular.element("body").addClass("component-dragging-start");
                    angular.element("#shadowDragIcon").show()
                            .offset({top: evt.pageY - 20, left: evt.pageX - 20 })
                    evt.preventDefault();
                    scope.$apply(function () {
                        if (attrs.dragModel) {
    ...
    app.directive("drop", ['$rootScope', "dragAndDrop", "$document", function ($rootScope, dragAndDrop, $document) {
        ...
        return {
            ...
            link: function (scope, $element, attrs) {
                ...
                angular.element("body").off("mousemove", mouseMove_mine);
                angular.element("body").off("mouseup", mouseUp_mine);

                angular.element("body").on("mouseup", mouseUp_mine);
                angular.element("body").on("mousemove", mouseMove_mine);
                

                function mouseMove_mine(evt) {
                    if (window.ON_ELEMENT_CLICK_FOR_DRAG) {
                        angular.element("#shadowDragIcon").offset({top: evt.pageY - 20, left: evt.pageX - 20 })
                    }
                };

                function mouseUp_mine(e) {
                    window.ON_ELEMENT_CLICK_FOR_DRAG = false;
                    angular.element("body").removeClass("component-dragging-start");
                    angular.element("#shadowDragIcon").hide();
                };
                ...
                if (attrs.dropMouseEnter) {
                    ...
                    angular.element($element).addClass("component-dragging-start");
